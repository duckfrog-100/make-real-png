...
function App() {
  ...
  React.useEffect(() => {
    const onNavigate = (event: Event) => { const to = (event as CustomEvent<string>).detail; window.history.pushState({}, '', to); setPath(window.location.pathname); };
...
function QRDownloadPanel({ record }: { record: ResultRecord }) { const [qr, setQr] = React.useState(''); const mobileUrl = React.useMemo(() => `${window.location.origin}/mobile-download/${record.id}`, [record.id]); const createQr = async () => setQr(await generateQRCode(mobileUrl)); return <section className="rounded-3xl bg-white p-5 shadow"><h2 className="text-xl font-extrabold">QR 모바일 저장</h2><p className="mt-2 text-sm text-slate-500">모바일에서 QR코드를 스캔하면 선택한 결과 이미지를 저장할 수 있습니다.</p><button onClick={createQr} className="mt-4 w-full rounded-2xl bg-emerald-600 px-5 py-3 font-bold text-white">QR코드 생성</button>{qr && <div className="mt-4 rounded-2xl border p-4 text-center"><img src={qr} className="mx-auto" /><p className="mt-3 break-all text-xs text-slate-500">{mobileUrl}</p></div>}</section>; }
